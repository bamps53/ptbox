...
def run(config_file, device_id, idx_fold):
    ...
    if config.model.load_from:
        checkpoint_path = config.model.load_from + '_fold{}'.format(idx_fold) + '/checkpoints/best.pth'
    else:
    ...
    if config.data.unique_only:
        callbacks.append(AccuracyCallback(prefix="accuracy_a", feature="unique"))
    else:
        callbacks.extend(
            [
    ...
    wandb.init(project=config.project, name='{}_fold{}'.format(exp_name, idx_fold), config=config, reinit=True)
    # model training
    runner.train(
        model=model,
        criterion=criterion,
