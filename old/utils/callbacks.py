...
#         assert fields == ("features", "targets"), "so far it works only for this name"
...
#         model_scale = state.input["features"].shape[-1] // state.input["targets"].shape[-1]
#         bbx1, bby1, bbx2, bby2 = rand_bbox(state.input[self.input_key].size(), self.lam)
#         bbx1_t, bby1_t, bbx2_t, bby2_t = bbx1 // model_scale, bby1 // model_scale, bbx2 // model_scale, bby2 // model_scale
#         state.input["features"][:, :, bbx1:bbx2, bby1:bby2] = \
#             state.input["features"][self.index, :, bbx1:bbx2, bby1:bby2]
#         state.input["targets"][:, :, bbx1_t:bbx2_t, bby1_t:bby2_t] = \
#             state.input["features"][self.index, :, bbx1_t:bbx2_t, bby1_t:bby2_t]
