...
def get_transforms(phase_config):
    ...
    if phase_config.Cutout.num_holes > 0:
        ...
        list_transforms.append(Cutout(num_holes, hole_size, hole_size))

    if phase_config.GridMask:
        list_transforms.append(
            OneOf([
                GridMask(num_grid=(3, 7), mode=0),
                GridMask(num_grid=(3, 7), mode=0),
                GridMask(num_grid=(3, 7), mode=1),
                GridMask(num_grid=(3, 7), mode=2),
            ], p=1)
        )
    ...
    if phase_config.All:
        ...
        list_transforms.append(
            OneOf([
                ...
                Cutout(num_holes, hole_size, hole_size),
                GridMask(num_grid=(3, 7), mode=0),
                GridMask(num_grid=(3, 7), mode=1),
                GridMask(num_grid=(3, 7), mode=2),
            ], p=1)
    ...
    if phase_config.AugMix:
        list_transforms.append(RandomAugMix(severity=1, width=1, p=1.))
